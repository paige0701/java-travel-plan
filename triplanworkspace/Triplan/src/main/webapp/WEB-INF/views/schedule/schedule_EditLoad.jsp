...
<c:forEach items="${StringArr}" var="td"  varStatus="status">
	...
	<c:if test="${status.index % collen != 0}"> <td class="jh_sc_${status.index % collen} <c:if test="${status.index % collen <= 3}">view</c:if>" <c:if test="${status.index > collen-1}">ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="dragleave(event)"</c:if> >${td}</td> </c:if>  <!-- 행의 나머치 컬럼갯수 -->
